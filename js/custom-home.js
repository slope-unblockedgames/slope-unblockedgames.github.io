...
function loadData() {
  ...
  fetch(`/data/${filename}.json?v=${date_tmp}`)
    .then((response) => response.json())
    .then((data) => {
      var listGame = [];
      var listCheck = [];
      ...
      listGame.forEach((item, index) => {
        ...
        if (isNumeric(tmp_str[0])) {
          ...
          if (
            index > 0 &&
            listGame[index].title.toLowerCase().split("")[0] !=
              listGame[index - 1].title.toLowerCase().split("")[0]
          ) {
      ...
      if (
        ...
        window.location.href.indexOf("tunnel-rush.html") == -1 &&
        window.location.href.indexOf("monkey-mart.html") == -1
      ) {
        loadGA();
        loadAds();
      }
...
function loadMainGame() {
  document.querySelector(".game-frame").innerHTML = `<iframe src="${
    document.querySelector(".game-frame").dataset.url
  }" id="gameframe" frameborder="0" style="height: 100%;width: 100%;"></iframe>`;
}
function playGame() {
  document.querySelector(
    ".game-frame"
  ).innerHTML = `<iframe allowfullscreen="true" src="${
    document.querySelector("#talpa-splash-button").dataset.url
  }" id="gameframe" frameborder="0" style="height: 100%;width: 100%;"></iframe>`;
  playBtnCode();
}
